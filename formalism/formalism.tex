...
\begin{theorem}[name=Constructability] \
  \begin{enumerate}
    ...
    \item For every $\EMV$, there exists $\AIMV$ such that
      $\AUEActionIter{\ZCursor{\EEHole}}{\EMV}{\AIMV}$.
  \end{enumerate}
\end{theorem}
...
\begin{theorem}[name=Determinism] \
  \begin{enumerate}
    ...
    \item If $\AUTActionIter{\ZMV}{\ZMV'}{\AMV}$ and $\AUTActionIter{\ZMV}{\ZMV''}{\AMV}$, then $\ZMV'
      = \ZMV''$.
  \end{enumerate}
\end{theorem}
...
\subsubsection{Iterated Actions}
The iterated action judgments are the same as in the untyped model.
